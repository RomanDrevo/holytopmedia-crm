...
var ScoreboardFTD = {
    ...
    processNewData: function(data) {
        ...
        $.each(data, function(i, employee) {
            var employee_row = this.$table.find("#employee_row_" + employee.id);
            console.log(employee);
            employee_row.find(".employee_goal_daily").html(employee.goal.daily);
            employee_row.find(".employee_goal_monthly").html(employee.goal.monthly);

            var dailyField = employee_row.find(".employee_daily_total");
            var monthlyField = employee_row.find(".employee_monthly_total");

            if (parseInt(dailyField.html()) != employee.dailyTotal && employee.dailyTotal != undefined) {
                hasNewDeposit = true;
                dailyField.html(employee.dailyTotal);
            }

            if (parseInt(monthlyField.html()) != employee.monthlyTotal && employee.monthlyTotal != undefined) {
                hasNewDeposit = true;
                monthlyField.html(employee.monthlyTotal);
            }
        }.bind(this));
        if (hasNewDeposit) {
